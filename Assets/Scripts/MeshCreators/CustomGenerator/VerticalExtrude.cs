...
public class VerticalExtrude : MeshCreator
{
    ...
    public void CopyFrom(VerticalExtrude copy)
    {
        height = copy.height;
    ...
    public override void RecalculateMesh()
    {
        ...
        for (int i = 0; i < points.Count; i++)
        {
            ...
            builder.AddVertex(new Vector3(points[i].x, 0, points[i].z), new Vector2(points[i].x, points[i].z));
        }
        for (int t = 0; t < triangles.Count; t += 3)
        {
            builder.AddTriangle(triangles[t + 2], triangles[t + 1], triangles[t]);
        ...
        {
            // TODO: Add uvs
            builder.AddVertex(new Vector3(points[i].x, height, points[i].z), new Vector2(points[i].x, points[i].z));
        }
        for (int t = 0; t < triangles.Count; t += 3)
        {
            builder.AddTriangle(n + triangles[t], n + triangles[t + 1],n + triangles[t + 2]);
        }
        ...
        for (int i = 0; i < points.Count; i++)
        {
            ...
            int v1 = builder.AddVertex(new Vector3(points[i].x, 0, points[i].z), new Vector2(0, 0));
            int v2 = builder.AddVertex(new Vector3(points[j].x, 0, points[j].z), new Vector2(horizontalDistance, 0));
            // back vertices:
            int v3 = builder.AddVertex(new Vector3(points[i].x, height, points[i].z), new Vector2(0, height));
            int v4 = builder.AddVertex(new Vector3(points[j].x, height, points[j].z), new Vector2(horizontalDistance, height));
            // Add quad:
