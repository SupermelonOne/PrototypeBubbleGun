	...
	public override void RecalculateMesh() {
        ...
        for (int i = 0; i < points.Count-1; i++) {
			...
			for (int j = 0; j<InputMesh.vertexCount; j++) {
				...
				Quaternion inputRotation = localOrientation[i];
				if (i != points.Count-2)
                    inputRotation = Quaternion.Slerp(localOrientation[i], localOrientation[i + 1], t);
				Vector3 rotatedXYModelCoordinate = inputRotation * inputV;
